...
void map::load_wsv(const char* file)
{
    ...
    for (; input_file.getline(buffer, 2048);)
    {
        ...
        case csv_line::entity_type::player_spawn:
            {
                // init player
                health_bar * h = new health_bar( {10, config::window::height - 25}, {123,21}, {0,0}, {41,7},"../resources/healthbar.png",p);
                p = new player({{line.x, line.y}}, {{48.0f, 48.0f}}, 100, 100);
                game::ui_components.push_back(h);
                game::ui_components.push_back(new food_bar( {10, config::window::height - h->get_size().width - 25}, {123,21}, {0,0}, {41,7},"../resources/foodbar.png",p));
