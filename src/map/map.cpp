...
void map::load_wsv(const char * file){
    ...
    for(unsigned int y = 0 ; y < nb_clusters_y ; y++) {
        for(unsigned int x = 0; x < nb_clusters_x ; x++) {
            clusters.push_back(cluster({static_cast<float>(y*ch),static_cast<float>((x+1)*cw),static_cast<float>((y+1)*ch),static_cast<float>(x*cw)}));
        }
    }
    ...
    for (; input_file.getline(buffer, 2048); ) {
        ...
        switch (line.type) {
            ...
            case csv_line::entity_type::player_spawn: {
                ...
                cam = camera({{ offset_x, offset_y }}, {{ static_cast<float>(config::viewport::width), static_cast<float>(config::viewport::height) }}, .8f, 1.5f, p);
            } break;
            default: {
