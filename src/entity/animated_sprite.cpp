...
animated_sprite::animated_sprite(vec2<float> _position, vec2<float> _size, vec2<uint32_t> _frame_resolution, vec2<float> _anchor,
                                 const char *image_src, const std::vector<uint32_t>& _animation_lengths, float _frame_time, uint32_t animation_id):
    entity(_position),
    sprite(_position, _size, {{0, animation_id}}, _frame_resolution, _anchor, image_src),
