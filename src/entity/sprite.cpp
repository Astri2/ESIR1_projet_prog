...
sprite::sprite(vec2<float> _position, vec2<float> _size, vec2<uint32_t> _sprite_offset, vec2<uint32_t> _sprite_resolution, vec2<float> _anchor, const char* image_src):
        entity(_position),  size(_size), sprite_offset(_sprite_offset), sprite_resolution(_sprite_resolution), anchor(_anchor)
{
